/**
 * @fileoverview ポータルスペースにメニュー一覧とグラフ表示（モバイル版）
 * - リソース：
 * - イベント：
 *
 * 【必要ライブラリ】
...
(function ($, globalConfig, portalGlobalConfig, portal) {
    ...
    kintone.events.on([
        ...
        // スペース上側の要素を取得
        var el = kintone.mobile.space.portal.getContentSpaceElement();
    ...
    function openTab(tabName) {
        if (tabName === 'nok-portal-tab') {
            ...
            $('.gaia-mobile-v2-space-spacetabview').hide();
        } else {
            $('.nok-portal-outer').hide();
            $('.gaia-mobile-v2-space-spacetabview').show();
        }
    }
