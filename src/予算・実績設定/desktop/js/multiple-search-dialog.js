...
*/
jQuery.noConflict();
(function ($, nokConfig, msdConfig, sncLib) {
    'use strict';
    ...
    function searchAndShowResult(dialogConf) {
        ...
        return new kintone.Promise(function (resolve, reject) {
            ...
            // 検索結果の件数が設定値以上の場合
            if (res > 0 && res > dialogConf.dialog.maxResults) {
                Swal.fire({
                    title: 'warning'
            ...
            if (selected_list_key_of_record.length > 0) {
                ...
                return getRecordOfSelectedList(dialogConf);
            } else {
                // ダイアログを閉じて終了
                Swal.close();
                return false;
                            ...
                            if (diffDays.values) {
                                if (diffDays.values.days > 0) {
                                    searchCondErrors.push({
                                        label: value.label,
                                        msg: hikiateKaijoConfig.messages.errorIncorrectDate
                                    })
                                }
                            }
                ...
                window.addEventListener("message", (function (event) {
                    return function field_set() {
                        ...
                        if (btnConf.target && btnConf.source) {
                            var record = tab.kintone.app.record.get();
...
})(jQuery, window.nokConfig, window.msdConfig, window.snc);
