...
(function ($, nokConfig) {
    ...
    * */
    kintone.events.on([
        ...
        if (event.type === 'mobile.app.record.create.submit') {
            // テーブル1行目にデータを登録
            // 役職
    ...
    * */
    kintone.events.on([
        ...
        'mobile.app.record.edit.show',
        'mobile.app.record.create.change.' + tantoJohoConfig.layerTable.code,
        'mobile.app.record.edit.change.' + tantoJohoConfig.layerTable.code,
    ], function (event) {
        let record = event.record;
        let layerTable = record[tantoJohoConfig.layerTable.code].value;
