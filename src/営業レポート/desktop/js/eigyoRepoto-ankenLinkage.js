/**
 * @fileoverview 営業活動レポート　案件管理アプリ連携
...
(function ($, config, configLinkage, configAnkenProcess, sncLib) {
	...
	/**
	 * 案件情報へ遷移するボタン を配置
	 *   顧客名検索コードをGETパラメーターで渡す
	 * @param {boolean} opt_isSendParam true:顧客名検索コードをGETパラメーターで渡す
	 */
	function setAnkenJohoTorokuMoveButton(opt_isSendParam) {
		var isSendParam = opt_isSendParam || false;
		// 案件管理アプリへ遷移するボタンの表示
		var submitButton = document.createElement('button');
		submitButton.id = 'submit_button';
		submitButton.innerHTML = sncLib.util.escapeHtml('案件情報登録');
		// 新デザインのクラスを設定
		$(submitButton).addClass('kintoneplugin-button-normal');
		// ボタンクリック時の処理
		submitButton.onclick = function () {
			...
			var url = '/k/' + config.anken.app + '/edit';
			if (isSendParam) {
				if (record && record[cfgEigyoRepotoFields.kokyakuId.code].value) {
					url += '?' + cfgEigyoRepotoFields.kokyakuId.code + '=' + record[cfgEigyoRepotoFields.kokyakuId.code].value;
				}
			}
		...
		submitLabel.innerHTML = '<BR>';
		$(submitLabel).addClass('kintoneplugin-label');
		// スペースにボタンを生成
		kintone.app.record.getSpaceElement('anken_space').appendChild(submitLabel);
		kintone.app.record.getSpaceElement('anken_space').appendChild(submitButton);
	}
	...
	*/
	kintone.events.on([
		...
		// 案件情報アプリへ遷移するボタンを表示
		setAnkenJohoTorokuMoveButton(true);
		return event;
	});
	...
	kintone.events.on([
		...
		// 案件進捗情報の更新
		if (updateAnkenProcessFields(event.record)) {
			// 成功した場合、削除処理を続行
			return event;
		} else {
			// 失敗した場合、削除処理をキャンセル
